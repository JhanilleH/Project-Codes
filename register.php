<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        if (count($errors) == 0) {
            ...
            $userquery="select * FROM currentuser WHERE username='".$username_login."'";
            $output=mysqli_query($conn, $userquery);

            $none=0;
            if ($value=mysqli_query($conn, $userquery))
                { 
            ...
            if ($rows == $none)
                ...
                {
                    $userim = "UPDATE currentuser SET username='".$username_login."' "; 
                    mysqli_query($conn,$userin);
                }
